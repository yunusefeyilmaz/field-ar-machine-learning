...
# For data collection from multiple processes
# (We don't need the lock with ProcessPool as each process returns complete results)
...
def process_image_with_weather(args):
    """
    Process a single satellite image with its corresponding weather data.
    This function is designed to be run in parallel by the ProcessPool.
    
    Args:
        args: Tuple containing (date_str, image_file, weather_data, weather_times)
        
    Returns:
        List of row data dictionaries ready to be added to the dataset
    """
    date_str, image_file, weather_data, weather_times = args
    rows = []
...
def build_dataset(num_processes=None):
    """
    Build the complete dataset combining satellite and weather data using multi-processing.
    
    Args:
        num_processes: Number of processes to use. If None, it will use the CPU count.
    """
    # Default to CPU count if num_processes not specified
    ...
    processing_tasks = [
        (date_str, image_file, weather_data, weather_times)        for date_str, image_file in satellite_dates.items()
    ]
    
    total_images = len(processing_tasks)
    print(f"Processing {total_images} satellite images with {num_processes} processes...")
    
    # Process images in parallel using ProcessPool
    all_rows = []
    
    # Use a ProcessPoolExecutor for true parallel processing
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_processes) as executor:
        # Use tqdm to track progress
        futures = {executor.submit(process_image_with_weather, task): task for task in processing_tasks}
        
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), 
                          desc="Processing images", unit="image"):
            task = futures[future]
            try:
                # Get result from this process and add to collective results
                rows = future.result()
                all_rows.extend(rows)  # No lock needed as we're in the main process
            except Exception as e:
                print(f"Error processing {task[1]}: {e}")
    
    # Convert all collected rows to DataFrame
...
if __name__ == "__main__":
    # You can specify the number of processes as an argument
    import argparse
    
    parser = argparse.ArgumentParser(description='Build water stress dataset using multi-processing')
    parser.add_argument('--processes', type=int, default=None, 
                        help='Number of processes to use (default: CPU count)')
    args = parser.parse_args()
    
    build_dataset(num_processes=args.processes)
